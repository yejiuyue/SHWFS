...
[row, column] = size(area);

subSizeR = row / subNum;
subSizeC = column / subNum;

valueAry = false(subNum); % 使用布尔型数组更加高效
center = zeros(subNum^2, 2); % 预分配足够的空间
S = 1;

for ri = 1:subNum
    rowStart = round(1 + (ri-1)*subSizeR);
    rowEnd = round(ri*subSizeR);
    for ci = 1:subNum
        colStart = round(1 + (ci-1)*subSizeC);
        colEnd = round(ci*subSizeC);
        
        temp = area(rowStart:rowEnd, colStart:colEnd);
        tempR = rowEnd - rowStart + 1; % 直接计算子矩阵的行列数
        tempC = colEnd - colStart + 1;

        if sum(temp(:)) / tempR / tempC >= 1
            c_row = (ri-1)*subSizeR + subSizeR/2;
            c_col = (ci-1)*subSizeC + subSizeC/2;
            center(S,:) = [c_col, c_row];
            valueAry(ri, ci) = true; % 更改成布尔型数组
            S = S + 1;
        end
    end
end

center(S:end,:) = []; % 移除预分配但未使用的空间
% end
